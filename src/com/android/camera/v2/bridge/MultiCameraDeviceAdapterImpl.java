...
public class MultiCameraDeviceAdapterImpl implements IMultiCameraDeviceAdapter,
    ...
    private static final Tag TAG = new Tag(
            MultiCameraDeviceAdapterImpl.class.getSimpleName());
    ...
    public void connectSelectedDevice(String deviceKey) {
        LogHelper.i(TAG, "connectSelectedDevice deviceKey " + deviceKey);
        throw new RuntimeException("Function is not ready.");
    }

    @Override
    public void disconnectAllRemoteCamera() {
        LogHelper.i(TAG, "onDisconnectButtonClicked");
    ...
    @Override
    public LinkedHashMap<String, IRemoteDevice> queryRemoteDevices() {
        if (mCrossMountAdapter == null) {
            mCrossMountAdapter = CrossMountAdapter.getDefaultAdapter(mActivity, this);
        }
        Device[] devices = mCrossMountAdapter.getAvailableDevices();
        return mDeviceInfoManager.createDeviceInfo(devices);
    }
    ...
    public void onFeatureStateUpdate(CrossMountAdapter.FeatureState featureState) {
        LogHelper.d(TAG, "onFeatureStateUpdate featureState = " + featureState);
        if (mCrossMountAdapter == null) {
            return;
        }
    ...
    public void onDiscoverResultUpdate(int reason) {
        LogHelper.d(TAG, "onDiscoverResultUpdate reason = " + reason);
        if (mCrossMountAdapter == null) {
            return;
        }
        Device[] devices = mCrossMountAdapter.getAvailableDevices();
        if (devices != null) {
            mStatusUpdateListener.onAvalibleDevicesUpdate(
                    mDeviceInfoManager.createDeviceInfo(devices));
        }
    }

    @Override
    public void onConnectionResultUpdate() {
        LogHelper.i(TAG, "onConnectionResultUpdate");
        if (mCrossMountAdapter == null) {
            return;
        }
        Device[] devices = mCrossMountAdapter.getAvailableDevices();
    ...
    @Override
    public void onServiceEventUpdate(CrossMountAdapter.ServiceEvent event,
            int value, String deviceId) {

        LogHelper.i(TAG, "onServiceEventUpdate(), event:" + event + ", value:"
                + value + ", deviceId:" + deviceId);

        switch (event) {
        case CAMERA_ORIENTATION:
            mStatusUpdateListener.onServiceEventUpdate(
                    REMOTE_CAMERA_ORIENTATION_CHANGE, value, deviceId);
            return;
        default:
            return;
        }
    ...
    private boolean isCrossMountEnabled() {
        ...
        LogHelper.d(TAG, "isCrossMountEnabled enabled = " + enabled);
        return enabled;
    }
    ...
    private class DeviceInfoManager {
        ...
        public LinkedHashMap<String, IRemoteDevice> createDeviceInfo(
                Device[] devices) {
            LinkedHashMap<String, IRemoteDevice> map = new LinkedHashMap<String, IRemoteDevice>();
            mDevicesList.clear();
            if (devices == null) {
                LogHelper.d(TAG, "createDeviceInfo devices is null, return directly.");
                map.putAll(mDevicesList);
                return map;
            }

            Service service = null;
            LogHelper.d(TAG, "createDeviceInfo devices.length "
                    + devices.length);
            ...
            for (int i = 0; i < devices.length; i++) {
                if ((service = getCameraService(devices[i])) != null) {
                    ...
                    boolean isValid = isCameraServiceValid(service);
                    if (!isValid) {
                        continue;
                    }
        ...
        private ArrayList<Integer> getLocalCameraIDList(Activity activity) {
            ...
            LogHelper.i(TAG, "getLocalCameraIDList localCameraList "
                    + localCameraList);
        ...
        private void initCameraIDMappingTable(Device[] devices) {
            ...
            for (int i = 0; i < devices.length; i++) {
                if ((service = getCameraService(devices[i])) != null) {
                    if (getCameraServiceState(service) == REMOTE_CAEMRA_STATUS_CONNECTED) {
                        ...
                        LogHelper.d(TAG,
                                "[initCameraIDMappingTable] get a connected camera id = " + id);
            ...
            LogHelper.i(TAG,
                    "[initCameraIDMappingTable] mCameraIDMappingList = "
                            + mCameraIDMappingList);
        }
        ...
        private Service getCameraService(Device device) {
            ...
            String cameraServiceId = mCrossMountAdapter
                    .getBuildInServiceId(BuildInServiceType.CAMERA);
            if (cameraServiceId != null) {
                // find if the device has the camera service or not.
                for (int i = 0; i < services.length; i++) {
                    String serviceId = services[i].getId();
                    LogHelper.d(TAG, "getCameraService serviceId = "
                            + serviceId);
                    if (cameraServiceId.equals(serviceId)) {
                        return services[i];
                    }
                }
            }
        ...
        private int getCameraServiceState(Service service) {
            ...
            LogHelper.d(TAG, "getServiceStateValue state " + state);
            switch (state) {
            case OCCUPIED:
        ...
        private boolean isCameraServiceValid(Service service) {
            int state = getCameraServiceState(service);
            return state == REMOTE_CAMERA_STATUS_AVALIABLE
                    || state == REMOTE_CAEMRA_STATUS_CONNECTING
                    || state == REMOTE_CAEMRA_STATUS_CONNECTED;
        }
    }
}

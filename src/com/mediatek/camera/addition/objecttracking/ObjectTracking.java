...
public class ObjectTracking extends CameraAddition implements OtListener {
    ...
    public ObjectTracking(ICameraContext context) {
        ...
        mICameraView = mICameraAppUi
                .getCameraView(SpecViewType.ADDITION_OBJECT_TRACKING);
    ...
    public boolean execute(ActionType type, Object... arg) {
        Log.i(TAG, "[execute] ActionType = " + type
                + "mIsObjectTrackingStarted = " + mIsObjectTrackingStarted
                + ", mIsParameterReady = " + mIsParameterReady
                + ", mPreviewState = " + mPreviewState);
        switch (type) {
        case ACTION_ON_LONG_PRESS:
            if (!mIsParameterReady
                    || PreviewState.STATE_STARTED != mPreviewState) {
                return false;
            }
            stopOT();
            startOT(arg[X], arg[Y]);
    ...
    public boolean execute(AdditionActionType type, Object... arg) {
        ...
        case ACTION_TAKEN_PICTURE:// take picture
            ...
            mISettingCtrl.onSettingChanged(
                    SettingConstants.KEY_OBJECT_TRACKING, "off");
            return false;
        ...
        case ACTION_ON_STOP_PREVIEW:
            if (mIsSupportIndicator && mIsObjectTrackingStarted) {
                ...
                if (mICameraDeviceManager.getCurrentCameraId() != mICameraDeviceManager
                        .getFrontCameraId()) {
                    mISettingCtrl.onSettingChanged(
        ...
        case ACTION_ON_START_PREVIEW:
            Log.i(TAG, "[execute] ACTION_ON_START_PREVIEW mOldX = " + mOldX
                    + ", isSupportOtCases()" + isSupportOtCases());
            if (needRestartOt()) {
                stopOT();
    ...
    public void onObjectTracking(Face face, Camera camera) {
        if (face == null
                || (face.score != OBJECT_TRACKING_SUCCEED
                && face.score != OBJECT_TRACKING_FAILED)) {
    ...
    private void startOT(Object arg0, Object arg1) {
        Log.i(TAG, "[startOT] mIsObjectTrackingStarted = "
        ...
        int[] pt = init(x, y);
        if (mIsSupportIndicator) {
            mICameraView.update(UPDATE_DISPLAY_ORIENTATION,
                    mIModuleCtrl.getDisplayOrientation());
        }
        // Record face detection status before stop FD.
        mFDStatusBackup = getFaceDetectStatus();
        mISettingCtrl.onSettingChanged(SettingConstants.KEY_CAMERA_FACE_DETECT,
                "off");
        mISettingCtrl.onSettingChanged(SettingConstants.KEY_OBJECT_TRACKING,
                "on");
        mISettingCtrl.onSettingChanged(SettingConstants.KEY_HEARTBEAT_MONITOR, "false");
    ...
    private int[] init(int x, int y) {
        ...
        mICameraView.update(SET_PREVIEW_WIDTH_HEIGHT, mPreviewWidth,
                mPreviewHeight);
    ...
    private void reStartOT(float x, float y) {
        Log.i(TAG, "[reStartOT] mIsObjectTrackingStarted = "
                + mIsObjectTrackingStarted);
        if (mIsObjectTrackingStarted || !isSupportOtCases()) {
            return;
        }
     ...
     */
    private void stopOT() {
        Log.i(TAG, "[stopOT] mIsObjectTrackingStarted = "
                + mIsObjectTrackingStarted);
        if (!mIsObjectTrackingStarted) {
            return;
        }
        ...
        if (mICameraDeviceManager.getCurrentCameraId() != mICameraDeviceManager
                .getFrontCameraId()) {
            mISettingCtrl.onSettingChanged(
                    SettingConstants.KEY_CAMERA_FACE_DETECT, mFDStatusBackup);
        }
        mISettingCtrl.onSettingChanged(SettingConstants.KEY_OBJECT_TRACKING,
                "off");
        mISettingCtrl.onSettingChanged(SettingConstants.KEY_HEARTBEAT_MONITOR, "true");
        mICameraDevice.applyParameters();
        mIModuleCtrl.startFaceDetection();
        mOldX = INIT_VALUE;
    }
    ...
    private void setMatrix() {
        Log.i(TAG, "[setMatrix] mPreviewWidth " + mPreviewWidth
                + ", mPreviewHeight" + mPreviewHeight);
        if (mPreviewWidth != 0 && mPreviewHeight != 0) {
            Matrix objectMatrix = new Matrix();
            Util.prepareMatrix(objectMatrix, false,
                    mIModuleCtrl.getDisplayOrientation(), mPreviewWidth,
                    mPreviewHeight);
    ...
    private boolean isSupportOtCases() {
        ...
        featureSupport = FEATURE_ON.equals(mISettingCtrl
                ...
                || FEATURE_ON.equals(mISettingCtrl
                        .getSettingValue(SettingConstants.KEY_PANORAMA)) || FEATURE_ON
                .equals(mISettingCtrl
                        .getSettingValue(SettingConstants.KEY_SLOW_MOTION));
        deviceReady = mICameraDeviceManager.getCurrentCameraId() != mICameraDeviceManager
                .getFrontCameraId()
                && mIModuleCtrl.isNonePickIntent()
                && mICameraDevice != null
                && mICameraDevice.getParameters() != null
                && mICameraDevice.getParameters().getMaxNumDetectedObjects() > 0
                && !("true".equals(mICameraDevice.getParameters().get(
                        Util.KEY_VIDEO_FACE_BEAUTY)))
                && !(ParametersHelper.isVsDofSupported(mICameraDevice
                        .getParameters()) && FEATURE_ON.equals(mISettingCtrl
                        .getSettingValue(SettingConstants.KEY_REFOCUS)));
        Log.i(TAG, "[isSupportOtCase] featureSupport = " + featureSupport
    ...
    private void calculateUnCropWidthAndHeight(int width, int height) {
        if (mICameraDevice.getParameters() != null
                && mICameraDevice.getParameters().getPreviewSize() != null) {
        ...
        if (((mUnCropHeight > mUnCropWidth) && ((mIModuleCtrl
                .getDisplayOrientation() == 0) || (mIModuleCtrl
                .getDisplayOrientation() == 180)))
                || ((mUnCropHeight < mUnCropWidth) && ((mIModuleCtrl
                        .getDisplayOrientation() == 90) || (mIModuleCtrl
                        .getDisplayOrientation() == 270)))) {
            int temp = mUnCropWidth;
            mUnCropWidth = mUnCropHeight;
            mUnCropHeight = temp;
        }
        Log.i(TAG, "[calculateUnCropWidthAndHeight] mUnCropWidth = "
                + mUnCropWidth + ", mUnCropHeight" + mUnCropHeight);
    }
    ...
    private boolean needRestartOt() {
        ...
        if (mOldX != INIT_VALUE && isSupportOtCases() && mIModuleCtrl != null) {
            need = (mIModuleCtrl.getPrevMode() == CameraModeType.EXT_MODE_PHOTO && (mIModuleCtrl
                    .getNextMode() == CameraModeType.EXT_MODE_PHOTO || mIModuleCtrl
                    .getNextMode() == CameraModeType.EXT_MODE_VIDEO));
            if (mIsTakePicture) {
                need = true;
            }
            Log.d(TAG,
                    "[needRestartOt]mIModuleCtrl.getPrevMode() = "
                            + mIModuleCtrl.getPrevMode()
                            + ", mIModuleCtrl.getNextMode()"
                            + mIModuleCtrl.getNextMode()
                            + ", mIsTakePicture = " + mIsTakePicture);
        }

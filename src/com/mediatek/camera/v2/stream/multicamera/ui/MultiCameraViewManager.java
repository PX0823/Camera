...
public class MultiCameraViewManager implements IMultiCameraViewManager {
    ...
    private static final Tag TAG = new Tag(
            MultiCameraViewManager.class.getSimpleName());
    ...
    public MultiCameraViewManager(IMultiCameraViewListener viewListener) {
        LogHelper.i(TAG, "[MultiCameraViewManager]");
    ...
    public void onOrientationChanged(int gsensorOrientation) {
        ...
        if (mOrientationCompensation != orientationCompensation) {
            LogHelper.i(TAG, "[onOrientationChanged] orientationCompensation "
    ...
    public void updatePreviewCamera(List<String> cameraId, boolean isRemoteOpened) {
        LogHelper.i(TAG, "[updatePreviewCamera] cameraId = " + cameraId
    ...
    @Override
    public void updateRecordingStatus(final boolean isStarted) {
        LogHelper.i(TAG, "[updateRecordingStatus] isStarted = " + isStarted);
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mIScanView.updateRecordingStatus(isStarted);
                mOperateView.updateRecordingStatus(isStarted);
            }
        });
    }
    ...
    public void updateCapturingStatus(final boolean isCapturing) {
        LogHelper.i(TAG, "[updateCapturingStatus] isCapturing = " + isCapturing);
        ...
        mActivity.runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                mDeviceTagView.updateCapturingStatus(isCapturing);
            }
        });
    }

    @Override
    public void updateFocusStatus(final boolean isFocusing) {
        LogHelper.i(TAG, "[updateFocusStatus] isFocusing = " + isFocusing);
        mActivity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                mOperateView.updateFocusStatus(isFocusing);
                mDeviceTagView.updateFocusStatus(isFocusing);
            }
        });
    }
    ...
    private DisplayManager.DisplayListener mDisplayListener = new DisplayManager.DisplayListener() {
        ...
        public void onDisplayChanged(int displayId) {
            ...
            LogHelper.i(TAG, "[onDisplayChanged]- mDisplayRotation:" + mDisplayRotation);
    ...
    private class OperateViewListener implements IOperateViewListener {
        ...
        public void onMultiPreviewButtonClick() {
            LogHelper.i(TAG, "[onMultiPreviewButtonClick]");
    ...
    public void startCountdown(int sec) {
        mOperateView.startCountdown(sec);
    }

    @Override
    public void cancelCountDown() {
        mOperateView.cancelCountDown();
    }
}

...
public class RecorderRenderer extends Renderer implements Runnable {
     ...
     */
    public RecorderRenderer(Activity activity, RendererConfig config) {
    ...
    public void setSurface(Surface surface) {
        LogHelper.i(TAG, "setRecrodingSurface surface = " + surface);
        if (surface == null) {
        ...
        synchronized (mReadyFence) {
            ...
            while (!mReady) {
                try {
                    ...
                    LogHelper.d(TAG, "InterruptedException ie:" + ie);
                }
            }
            ...
            updateRendererSize(surfaceWidth, surfaceHeight, true);
        }
    }
    ...
    public void start() {
        synchronized (mReadyFence) {
            LogHelper.i(TAG, "startRecording begin mEncoderHandler = " + mEncoderHandler);
        ...
        LogHelper.i(TAG, "startRecording end");
    }

    /**
     * when new frame is available, swap buffer to mediacodec (call from GL thread).
     */
    @Override
    public void draw() {
        synchronized (mReadyFence) {
            if (!mReady || !mIsCanRecordFirstFrame) {
                LogHelper.i(TAG, "mReady = " + mReady + " mIsCanRecordFirstFrame = "
                        + mIsCanRecordFirstFrame);
                return;
            }
            long presentationTimeNs = super.getPreviewStWrapper().getTimeStamp();
            if (mEncoderHandler != null) {
                ...
                mEncoderHandler.obtainMessage(EncoderHandler.MSG_FRAME_AVAILABLE,
                        ...
                        super.getTextureId()).sendToTarget();
    ...
    public void stop() {
        synchronized (mReadyFence) {
            LogHelper.i(TAG, "stopRecording mReady = " + mReady + " mIsStopRecrodingReceived = "
    ...
    public void run() {
        ...
        synchronized (mReadyFence) {
            Looper.prepare();
            LogHelper.i(TAG, "new EncoderHandler()");
        ...
        LogHelper.i(TAG, "Encoder thread exiting");
        synchronized (mReadyFence) {
    ...
    @Override
    public void release() {
        LogHelper.i(TAG, "releaseSurface");
    }

    private void updateRendererSize(int width, int height, boolean bottomIsMainCam) {
        LogHelper.i(TAG, "updateRendererSize width = " + width + " height = " + height);
    ...
    private void initVertexData(float width, float height, boolean bottomIsMainCam) {
        mVtxBuf = GLUtil.createFloatBuffer(mVtxBuf, GLUtil.createFullSquareVtx(width, height));
    ...
    private class EncoderHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            LogHelper.i(TAG, "handleMessage(" + msg + ")");
            switch (msg.what) {
            ...
            case MSG_QUIT:
                if (mRecordingEGLSurface != null) {
                    ...
                    LogHelper.i(TAG, "RecorderRendere swap buffer total count : "

...
public class RendererManager {
    ...
    private boolean mIsFristFrameArrive = true;

    private Renderer mRecorderRenderer; // for VR
     ...
     */
    public void init() {
        LogHelper.i(TAG, "[init]+ mEglHandler = " + mEglHandler);
        if (mEglHandler != null) {
     ...
     */
    public void unInit() {
        LogHelper.i(TAG, "[unInit]+ mEglHandler = " + mEglHandler);
    ...
    public void addRenderer(RendererConfig config) {
        LogHelper.d(TAG, "<addRenderer> type:" + config.getType() + ",name:" + config.getName());
    ...
    public void deleteRender(String name) {
        LogHelper.d(TAG, "<deleteRender> name:" + name);
    ...
    public void setRendererImage(Bitmap bitmap, String name) {
        LogHelper.d(TAG, "<setRendererImage> bitmap:" + bitmap + ",name:" + name);
    ...
    public void setPreviewSize(Size previewSize, String name) {
        LogHelper.i(TAG, "[setPreviewSize] +, previewSize:" + previewSize + ",name:" + name);
        ...
        waitDone();
        LogHelper.i(TAG, "[setPreviewSize]-");
    }
    ...
    public void setPreviewSurface(Surface surface) {
        LogHelper.i(TAG, "[setPreviewSurface]+");
    ...
    public void setCaptureSize(Size captureSize, String name) {
        ...
        LogHelper.i(TAG, "[setCaptureSize]-");
    }
    ...
    public void setCaptureSurface(Surface surface) {
        LogHelper.i(TAG, "setCaptureSurface surface:" + surface);
    }
    ...
    public void setSnapshotSurface(Surface surface) {
        LogHelper.d(TAG, "setSnapshotSurface,surface:" + surface);
    ...
    public void setRecordingSurface(Surface surface) {
        LogHelper.i(TAG, "setRecordingSurface surfacee:" + surface);
    ...
    public void startRecording() {
        LogHelper.i(TAG, "[startRecording]+ mRecorderRenderer:" + mRecorderRenderer);

        Assert.assertNotNull(mRecorderRenderer);

        mRecorderRenderer.start();
    ...
    public void stopRecording() {
        LogHelper.i(TAG, "[stopRecording]+ mRecorderRenderer:" + mRecorderRenderer
                + " mEglHandler:"
                + mEglHandler);
        ...
        mEglHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (mEglSurface == null || mEglSurface == EGL10.EGL_NO_SURFACE) {
                    LogHelper.i(TAG, "createWindowSurface error eglError = " + mEgl.eglGetError());
    ...
    private boolean waitDone() {
        ...
        synchronized (waitDoneLock) {
            ...
            try {
                ...
                LogHelper.v(TAG, "waitDone interrupted");
                return false;
            }
        }
        return true;
    ...
    private class EglHandler extends Handler {
        ...
        private Renderer mScreenRenderer; // for PV
        private Renderer mSnapshotRenderer; // for CAP/VSS
         ...
         */
        public SurfaceTexture getPreviewSurfaceTexture(String name) {
            LogHelper.d(TAG, "<getPreviewSurfaceTexture> name:" + name + ",size:" +
                    mRenderArray.size());
                ...
                for (Renderer render : mRenderArray) {
                    ...
                    if (name.equals(config.getName())) {
                        if (config.getType() == RendererType.RENDERER_ALPHA_VIDEO
                                || config.getType() == RendererType.RENDERER_PREVIEW) {
                            if (render.getPreviewStWrapper() == null) {
                                try {
                                    mRenderLock.wait();
                                } catch (InterruptedException e) {
                                    LogHelper.w(TAG, "unexpected interruption");
                                }
                            }
         ...
         */
        public SurfaceTexture getCaptureSurfaceTexture(String name) {
            LogHelper.d(TAG, "<getCaptureSurfaceTexture> name:" + name);
            synchronized (mRenderLock) {
                for (Renderer render : mRenderArray) {
                    RendererConfig config = render.getConfig();
                    if (name.equals(config.getName())) {
        ...
        public void setRenderRect(AnimationRect graphicRect, String name) {
            LogHelper.d(TAG, "<setRenderRect> name:" + name + ",rect:" + graphicRect.getRectF());
         ...
         */
        public boolean isContainsRender(String name) {
            LogHelper.d(TAG, "<isContainsRender> name:" + name);
            synchronized (mRenderLock) {
                for (Renderer render : mRenderArray) {
                    RendererConfig config = render.getConfig();
                    if (name.equals(config.getName())) {
                        LogHelper.d(TAG, "<isContainsRender> find render, name:" + name);
                        return true;
                    }
                }
                LogHelper.d(TAG, "<isContainsRender> can't find render, name:" + name);
                return false;
            }
        }
        ...
        private void addRenderer(RendererConfig renderConfig) {
            LogHelper.i(TAG, "[addRenderer]+ name = " + renderConfig.getName() + ",type:"
            ...
            for (Renderer render : mRenderArray) {
                ...
                if (renderConfig.getName().equals(config.getName())) {
                    LogHelper.d(TAG, "exit same name, add fail!!!!");
                    return;
                }
            }
            ...
            synchronized (mRenderLock) {
                Renderer render;
                if (renderConfig.getType() == RendererType.RENDERER_PREVIEW) {
                    render = RendererFactory.createRenderer(mActivity, renderConfig);
                } else {
                    Assert.assertTrue(false);
                    return;
                }
            ...
            updateMultiRenderer();
            LogHelper.i(TAG, "<createRenderers> success, size:" + mRenderArray.size());
        }
            ...
            synchronized (mRenderLock) {
                for (Renderer render : mRenderArray) {
                    ...
                    if (name.equals(config.getName())) {
                        ...
                        LogHelper.d(TAG, "<deleteRenderer> delete success, size:" +
                                mRenderArray.size());
                        return;
                    }
                }
                LogHelper.d(TAG, "not exist this render, delete fail!!!");
                Assert.assertTrue(false);
            }
            updateMultiRenderer();
        }
        ...
        private void initialize() {
            LogHelper.i(TAG, "[initialize]+ mSurfaceTextureHandler = " + mSurfaceTextureHandler);
            synchronized (mRenderLock) {
                if (mSurfaceTextureHandler == null) {
                    ...
                    RendererConfig screenConfig = new RendererConfig("screen",
                            RendererType.RENDERER_SCREEN);
                    mScreenRenderer = RendererFactory.createRenderer(mActivity, screenConfig);
                    mScreenRenderer.init();

                    // snapshot renderer
                    RendererConfig snapshotConfig = new RendererConfig("snapshot",
                            RendererType.RENDERER_SNAPSHOT);
                    mSnapshotRenderer = RendererFactory.createRenderer(mActivity, snapshotConfig);
        ...
        private void setPreviewSize(Size renderSize, String name) {
            LogHelper.d(TAG, "<setPreviewSize> renderSize:" + renderSize + ",name:" + name);
            ...
            for (Renderer render : mRenderArray) {
                ...
                if (name.equals(config.getName())) {
                    ...
                    render.setNeedPortrait(true);
                    render.setRendererSize(width, height);
                    ...
                    if (config.getType() == RendererType.RENDERER_ALPHA_VIDEO
                        ...
                        if (stWrapper == null) {
                            ...
                            LogHelper.d(TAG, "<setPreviewSize> set new Preview StWrapper:"
                                    + stWrapper);
                        } else if (stWrapper.getWidth() != width ||
                                stWrapper.getHeight() != height) {
                            stWrapper.setDefaultBufferSize(width, height);
                        } else {
                            LogHelper.d(TAG, "same width and height");
                        }
                    }
                }
            }

            SurfaceTextureWrapper stWrapper = null;
            for (Renderer render : mRenderArray) {
                if (render.getPreviewStWrapper() != null) {
                    stWrapper = render.getPreviewStWrapper();
                    break;
                }
            }
            if (stWrapper == null) {
                LogHelper.i(TAG, "[setPreviewSize] error, null texture wrapper!");
                return;
            }
            ...
            LogHelper.i(TAG, "[setPreviewSize] end");
        }

        private void setCaptureSize(Size captureSize, String name) {
            LogHelper.i(TAG, "setCaptureSize, captureSize:" + captureSize + ",name:" + name);

            for (Renderer render : mRenderArray) {
                RendererConfig config = render.getConfig();
                if (name.equals(config.getName())) {
                    if (config.getType() == RendererType.RENDERER_PREVIEW
        ...
        private void setPreviewSurface(Surface surface) {
            if (mScreenRenderer != null) {
                ...
                mIsFristFrameArrive = true;
            }
        }

        private void releaseRenderers() {
            LogHelper.i(TAG, "[releaseRenderers]+");
            synchronized (mRenderLock) {
        ...
        private void createRecordRenderer() {
            synchronized (mRenderLock) {
                RendererConfig config = new RendererConfig("recorder",
                        RendererType.RENDERER_RECORDER);
                mRecorderRenderer = RendererFactory.createRenderer(mActivity, config);
        ...
        private void setRecordingSurface(Surface recordingSurface) {
            synchronized (mRenderLock) {
                ...
                mRecorderRenderer.setSurface(recordingSurface);
                mRenderLock.notifyAll();
            }
        }
        ...
        private boolean doTimestampSync() {
            ...
            for (Renderer render : mRenderArray) {
                ...
                if (config.getType() == RendererType.RENDERER_PREVIEW
                    ...
                    if (wrapper != null && wrapper.isNewTimeStamp()) {
        ...
        private void draw() {
            synchronized (mRenderLock) {
                ...
                if (doTimestampSync() && !mBlockingForPvSizeChange) {
                    ...
                    if (mRecorderRenderer != null) {
                        mRecorderRenderer.setTextureId(mPreviewFrameBuffer.getFboTexId());
                        mRecorderRenderer.draw();
                    }

                    if (mIsFristFrameArrive) {
                        mRendererCallback.onFristFrameAvailable();
                        mIsFristFrameArrive = false;
                    }
                    // draw to screen
                    mScreenRenderer.setTextureId(mPreviewFrameBuffer.getFboTexId());
                    ...
                    if (mFrameCount % INTERVALS == 0) {
                        ...
                        LogHelper.i(TAG, "[Preview] Drawing frame, fps = " + fps
        ...
        private void doSnapshot() {
            LogHelper.d(TAG, "<doSnapshot> width:" + mRendererTexWidth + ",height:"
                    + mRendererTexHeight);

            mSnapshotRenderer.setNeedPortrait(true);
            mSnapshotRenderer.setRendererSize(mRendererTexWidth, mRendererTexHeight);
            mSnapshotRenderer.setSurface(mSnapshotSurface);
            ...
            mSnapshotRenderer.setTextureId(mPreviewFrameBuffer.getFboTexId());
            mSnapshotRenderer.draw();
        }
        ...
        private int initBitmapTexture(Bitmap bitmap) {
            ...
            LogHelper.i(TAG, "Renderer initBitmapTexture glGenTextures num = " + 1);
            ...
            for (Renderer render : mRenderArray) {
                ...
                if (para.mName.equals(config.getName())) {
                    if (config.getType() == RendererType.RENDERER_IMAGE) {
                        render.setTextureId(imageTexId);
                        return;
                    } else {
                        Assert.assertTrue(false);
                    }
        ...
        private void updateMultiRenderer() {
            int size = mRenderArray.size();
            if (size == 0) {
                LogHelper.d(TAG, "<updateMultiRenderer> size is 0");
                return;
            }
            if (size == 1) {
                mRenderArray.get(0).setMultiPreview(false);
                LogHelper.d(TAG, "<updateMultiRenderer> size is 1");
            } else {
                int num = 0;
                for (Renderer render : mRenderArray) {
                    if (render.isValidRenderRect()) {
                        num++;
                    }
                }
                LogHelper.d(TAG, "<updateMultiRenderer> render num is:" + num);
                if (num > 1) {
                    for (Renderer render : mRenderArray) {
                        render.setMultiPreview(true);
                    }
                } else {
                    for (Renderer render : mRenderArray) {
                        render.setMultiPreview(false);
                    }
                }
            }
        }

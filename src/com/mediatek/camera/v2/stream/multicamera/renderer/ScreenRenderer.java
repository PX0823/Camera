...
public class ScreenRenderer extends Renderer implements Runnable {
    private static final Tag TAG = new Tag(
            ScreenRenderer.class.getSimpleName());
    ...
    public void setSurface(Surface surface) {
        LogHelper.i(TAG, "setSurface surface = " + surface);
        ...
        synchronized (mReadyFence) {
            if (mScreenHandler != null && mIsEGLSurfaceReady) {
                ...
                mScreenHandler.obtainMessage(ScreenHandler.MSG_FRAME_AVAILABLE,
                        super.getTextureId(), 0).sendToTarget();
    ...
    public void run() {
        Looper.prepare();
        synchronized (mReadyFence) {
            LogHelper.i(TAG, "Screen renderer thread started!");
            mScreenHandler = new ScreenHandler();
            mReady = true;
            mReadyFence.notify();
        }
        Looper.loop();
        LogHelper.i(TAG, "Screen renderer thread exiting!");
        synchronized (mReadyFence) {
    ...
    private final DisplayManager.DisplayListener mDisplayListener =
            new DisplayManager.DisplayListener() {
                ...
                public void onDisplayChanged(int displayId) {
                    LogHelper.i(TAG, "onDisplayChanged displayId = " + displayId);
                    checkDisplayRotation();
                }
    ...
    private void waitRendererThreadActive() {
        synchronized (mReadyFence) {
            ...
            while (!mReady) {
                try {
                    LogHelper.i(TAG, "wait for screen renderer thread ready, current mReady = "
                            + mReady);
                    mReadyFence.wait();
    ...
    private class ScreenHandler extends Handler {
        ...
        private void doSetupEGLSurface() {
            LogHelper.i(TAG, "handleSetupSurface  mEglCore = " + mEglCore +
        ...
        private void doUpdateEGLSurface() {
            LogHelper.i(TAG, "updateEGLSurface mPreviewEGLSurface = " + mPreviewEGLSurface);
            if (mPreviewEGLSurface != null) {
        ...
        private void doReleaseSurface() {
            LogHelper.i(TAG, "doReleaseSurface");
            if (mPreviewEGLSurface != null) {
                mPreviewEGLSurface.makeNothingCurrent();
                mPreviewEGLSurface.releaseEglSurface();

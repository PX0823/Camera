...
public class CfbStreamControllerImpl implements ICfbStreamController, ICaptureStream {
    private static final Tag TAG = new Tag(CfbStreamControllerImpl.class.getSimpleName());
    ...
    public void openStream(StreamStatusCallback streamStatusCallback) {
        ...
        if (DEBUG) {
            LogHelper.i(TAG, "[openStream] ----,mCameraEffect = " + mCameraEffect);
        }
    }
    ...
    public void startCapture(CaptureStatusCallback captureStatusCallback) {
        if (DEBUG) {
            LogHelper.d(TAG, "[startCapture] captureStatusCallback = " + captureStatusCallback
    ...
    public void closeStream() {
        if (DEBUG) {
            LogHelper.d(TAG, "[closeStream] mCameraEffectHalSession = " + mCameraEffectHalSession);
        }
        mIsSizeChange = true;
    ...
    public boolean updateCaptureSize(Size pictureSize, int pictureFormat) {
        LogHelper.d(TAG, "[updateCaptureSize] picturesize's width = " + pictureSize.getWidth()
        ...
        LogHelper.i(TAG, "[updateCaptureSize],mOutPutSurfaces = " + mOutPutSurfaces);
        return true;
    }

    @Override
    public Map<String, Surface> getCaptureInputSurface() {
        LogHelper.d(TAG, "[getCaptureInputSurface]+++++, mIsInVideoMode = " + mIsInVideoMode
        ...
        if (inputSurfaces.size() != 0) {
            for (int i = 0; i < inputSurfaces.size(); i++) {
                ...
                LogHelper.d(TAG, "getCaptureInputSurface ,surface = " + inputSurfaces.get(i));
            }
        }
        return mInputSurface;
    }
    ...
    private StateCallback mEffectHalStateCallback = new StateCallback() {
        ...
        public void onError(CameraEffect effect, int error) {
            LogHelper.i(TAG, "[onError] effectHal = " + effect + ",error = " + error);
        ...
        public void onDisconnected(CameraEffect effect) {
            LogHelper.i(TAG, "[onDisconnected] effectHal = " + effect);
            mStreamStatusCallback.onStreamError();
        }
    };

...
public class SettingCharacteristics {
    ...
    private static final String FACE_BEAUTY_EFFECT_NAME = "FaceBeauty";
    ...
    public List<String> getSupportedValues(String key) {
        ...
        case SettingKeys.ROW_SETTING_FACEBEAUTY_SKIN_COLOR:
            List<String> faceBeautyValues = getFaceBeautySupportedValues();
            // If face beauty don't have support values,means current project
            // don't support this, need don't show the face beauty properties.
            if (faceBeautyValues != null && faceBeautyValues.size() > 0) {
                supportedValues = new ArrayList<String>(3);
        ...
        case SettingKeys.ROW_SETTING_FACE_BEAUTY:
            supportedValues = getFaceBeautySupportedValues();
            break;
        ...
        case SettingKeys.ROW_SETTING_SINK_CAMERA:
            if (FeatureSwitcher.isCrossMountSupported()) {
                supportedValues = new ArrayList<String>(2);
                supportedValues.add("on");
                supportedValues.add("off");
            }
        ...
        LogHelper.i(TAG, "camera:" + mCameraId + ", key:" + key + ", " +
    ...
    private List<String> getSupportedIsoValues(Range<Integer> range) {
        ...
        if (range != null) {
            ...
            LogHelper.i(TAG, "minIso:" + minIso + ", maxIso:" + maxIso);
        }
        return supportedValues;
    }
    ...
    private List<Integer> getAvailableCapablities() {
        ...
        if (availableCaps == null) {
            LogHelper.i(TAG, "The camera " + mCameraId + " available capabilities is null");
        ...
        LogHelper.i(TAG, "The camera " + mCameraId + " available capabilities are:" + capString);
        return capList;
    }
    ...
    private List<String> getFaceBeautySupportedValues() {
        int majorVersion = 0;
        int minorVersion = 0;
        int versionSize = -1;
        List<String> supportedValues = new ArrayList<String>();
        CameraEffectManager manager = new CameraEffectManager(mContext);
        boolean isEffectHalSupported = manager.isEffectHalSupported();

        if (isEffectHalSupported) {
            try {
                List<EffectHalVersion> versions = manager
                                .getSupportedVersion(FACE_BEAUTY_EFFECT_NAME);
                versionSize = versions.size();
                // size equals 0 means get supported version not porting in native,
                // so assume support FB.
                if (NOT_SUPPORT_EFFECT_FEATURE == versionSize) {
                    supportedValues.add("on");
                    supportedValues.add("off");
                } else {
                    for (int i = 0; i < versionSize; i++) {
                        EffectHalVersion versionValue = versions.get(i);
                        majorVersion = versionValue.getMajor();
                        minorVersion = versionValue.getMinor();
                        boolean isMajorEnable = NOT_SUPPORT_EFFECT_FEATURE != majorVersion;
                        boolean isMinorEnable = NOT_SUPPORT_EFFECT_FEATURE != minorVersion;
                        // if both major version and minor version equals zero means not
                        // support FB
                        if (isMajorEnable || isMinorEnable) {
                            supportedValues.add("on");
                            supportedValues.add("off");
                        }
                    }
        ...
        LogHelper.i(TAG, "getFaceBeautySupportedValues,isEffectHalSupported = "
                        + isEffectHalSupported + ",versionSize = " + versionSize

...
public class MultiCameraMode extends AbstractCameraMode implements ImageCallback {
    ...
    private static final Tag TAG = new Tag(
            MultiCameraMode.class.getSimpleName());
    ...
    private static final long SHUTTER_BUTTON_TIMEOUT = 1000L; // 500ms

    private boolean mIsCapturing = false;
    private boolean mPaused = true;
    private boolean mNeedUpdatePreviewCameraId = false;

    private int mJpegOrientation = 0;
    private int mOrientation = 0;
    private int mRemoteJpegOrientation = 0;
    ...
    public MultiCameraMode(AppController app, ModuleListener moduleListener) {
        ...
        mSettingCtroller.addRule(SettingKeys.KEY_MULTI_CAMERA_SINK, SettingKeys.KEY_VIDEO_QUALITY,
                new VideoQualityRule(mSettingCtroller));
    }
    ...
    public void configuringSessionOutputs(List<Surface> sessionOutputSurfaces, String cameraId) {
        LogHelper.i(TAG, "[configuringSessionOutputs] cameraId = " + cameraId
                        + ",preview camera = " + mPreviewCameraId);
        updatePreviewSurfaces();
        //If current is multipreview,not need update the capture surface,
        // the reason is current picture surface use snapshot surface.
        if (mPreviewCameraId.size() == 1) {
            updatePictureSize();
        ...
        LogHelper.i(TAG, "[configuringSessionOutputs]suface size: " + sessionOutputSurfaces.size());
    }
    ...
    public void configuringSessionRequests(Map<RequestType, Builder> requestBuilders,
            String cameraId) {
        LogHelper.i(TAG, "[configuringSessionRequests] cameraId: " + cameraId);
        ...
        while (iterator.hasNext()) {
            ...
            LogHelper.i(TAG, "requestType = " + requestType + " request number = " + keySet.size());
            switch (requestType) {
            case PREVIEW:
    ...
    public void updatePreviewCamera(List<String> previewCameraId, List<String> hasOpenedCamera) {
        ...
        LogHelper.i(TAG, "[updatePreviewCamera] previewCameraId = " + previewCameraId
                + ",hasOpenedCamera = " + hasOpenedCamera + ",mPaused = " + mPaused);
    ...
    public void onServiceEventUpdate(int event, int value, String cameraId) {
        LogHelper.d(TAG, "[onServiceEventUpdate] event = " + event + ",value = " + value);
        if (event == IMultiCameraDeviceAdapter.REMOTE_CAMERA_ORIENTATION_CHANGE) {
            mRemoteJpegOrientation = value;
        }

        mISinkStreamCtrl.onServiceEventUpdate(event, value, cameraId);
    }
    ...
    public CaptureCallback getCaptureCallback() {
        return new CaptureCallback() {
            ...
            public void onCaptureFailed(CameraCaptureSession session, CaptureRequest request,
                ...
                LogHelper.i(TAG, "[onCaptureFailed],reson = " + failure.getReason()
    ...
    protected Size getCaptureSize() {
        ...
        if (mIsRecording) {
            ...
            if (MultiCameraUtil.isLocalSinglePreview(mPreviewCameraId)) {
                ...
                // The picture size just use the remote camera picture size which get from
                // CameraCharacteristics.
                for (String camera : mPreviewCameraId) {
                    if (!MultiCameraUtil.isLocalCamera(camera)) {
                        pictureSize = MultiCameraUtil.getRemoteSupportedMaxPictureSize(mActivity,
                                        camera);
                    }
                }
        ...
        LogHelper.d(TAG, "[getCaptureSize] recording = " + mIsRecording + ",Size = " + pictureSize);

        // create the snap shot surface for multicamera preview do snap shot,
        // so the format is RGBA_8888
        ...
        if (mCaptureStreamCallback == null) {
            mCaptureStreamCallback = new CaptureStreamCallback() {
                ...
                public void onCaptureCompleted(ImageInfo imageInfo) {
                    LogHelper.i(TAG, "[onCaptureCompleted] +++++,mIsRecording = " + mIsRecording
                            + ",mIsCapturing = " + mIsCapturing + ",mNeedUpdatePreviewCameraId = "
                            + mNeedUpdatePreviewCameraId);
                    ...
                    if (mIsCapturing) {
                        // reset the capture status, must set before call
                        // doUpdatePreviewArea().
                        // because doUpdatePreviewArea() will judge the
                        // mIsCaptring parameters.
                        mIsCapturing = false;
                        if (!mIsRecording && mNeedUpdatePreviewCameraId) {
                            doUpdatePreviewArea();
                        }
                    }
                    ...
                    if (data != null) {
                        ContentValues values = MultiCameraUtil.getCaptureContentValues(width,
                                        heigth, getJpegOrientation(data),
        ...
        if (mRecordStreamCallback == null) {
            mRecordStreamCallback = new RecordStreamStatus() {
                ...
                public void onRecordingStoped() {
                    ...
                    mSettingCtroller.doSettingChange(SettingKeys.KEY_VIDEO,
                            "off", false);
                    ContentValues contentValues = MultiCameraUtil
                            .getVideoContentValues(
                                    mAppController.getActivity(),
                                    mCameraCamcorderProfile,
                                    mLocationManager.getCurrentLocation(),
                                    mRecordingRotation, mVideoTempPath,
                                    mStorageService.getFileDirectory());
                    mCameraServices.getMediaSaver().addVideo(mVideoTempPath,
                            contentValues, mMediaSavedListener,
                            mAppController.getActivity().getContentResolver());
                    mVideoTempPath = null;
    ...
    protected OnFileSavedListener mMediaSavedListener = new OnFileSavedListener() {
        ...
        public void onMediaSaved(Uri uri) {
            LogHelper.i(TAG, "onMediaSaved uri = " + uri);
    ...
    private void updateCaptureSurfaces() {
        ...
        LogHelper.d(TAG, "[updateCaptureSurfaces] : " + mCaptureSurface);
    }
    ...
    private void updatePreviewSurfaces() {
        ...
        LogHelper.d(TAG, "[updatePreviewSurfaces] : " + mPreivewSurface);
    }
    ...
    private void doUpdatePreviewArea() {
        ...
        // but when capture done,need update the preview camera id.
        if (!mIsCapturing) {
            ...
            mNeedUpdatePreviewCameraId = false;
        } else {
            // if current is capturing and also need update the preview camera,
            // just keep the action when capture done.
            mNeedUpdatePreviewCameraId = true;
        }
        LogHelper.i(TAG, "[doUpdatePreviewArea],mIsCapturing = " + mIsCapturing);
    }

    private Size getCameraCaptureSize(String cameraId) {
        ISettingServant settingServant = mSettingCtroller
                .getSettingServant(cameraId);
        SettingItem captureSizeItem = settingServant
                .getSettingItem(SettingKeys.KEY_PICTURE_SIZE);
        if (captureSizeItem == null) {
            return null;
        } else {
            String size = captureSizeItem.getValue();
            LogHelper.i(TAG, "getCameraCaptureSize camera id:" + cameraId
                    + " captureSizeString:" + size);
            if (size != null) {
    ...
    private void configurePreviewRequest(CaptureRequest.Builder requestBuilder, String cameraId) {
        LogHelper.i(TAG, "[configurePreviewRequest] cameraId = " + cameraId);
        Surface previewSurface = mPreivewSurface.get(cameraId);
        ...
        if (previewSurface.isValid()) {
            LogHelper.i(TAG, "[configurePreviewRequest] previewSurface = " + previewSurface);
    ...
    private void configureCaptureRequests(CaptureRequest.Builder requestBuilder, String cameraId) {
        Surface captureSurface = mCaptureSurface.get(cameraId);
        LogHelper.i(TAG, "[configureCaptureRequests] captureSurface = " + captureSurface
                        + ",cameraId = " + cameraId);
    ...
    private void photoShutterButtonClick() {
        LogHelper.i(TAG, "photoShutterButtonClick, mCurrentCameraId = " + mPreviewCameraId);
    ...
    private void videoShutterButtonClick() {
        LogHelper.i(TAG, "[videoShutterButtonClicked] mIsRecording = " + mIsRecording);
        mAppUi.setAllCommonViewEnable(false);
        if (mIsRecording) {
            // Stop VR
            stopRecording();
            ...
            LogHelper.i(TAG, "[videoShutterButtonClicked] mCameraCamcorderProfile = "
                    + mCameraCamcorderProfile);
    ...
    private void startRecording() {
        ...
        updatePreviewSize(new PreviewSurfaceCallback() {
            ...
            public void onPreviewSufaceIsReady(boolean surfaceChanged) {
                LogHelper.i(TAG, "startRecording onPreviewSufaceIsReady");
                mModuleListener.requestChangeSessionOutputs(true);
    ...
    private void stopRecording() {
        LogHelper.i(TAG, "[stopRecording]+,mPaused = " + mPaused);
        mIsRecording = false;
        releaseAudioFocus();
        if (mPaused) {
        ...
        updatePreviewSize(new PreviewSurfaceCallback() {
            @Override
            public void onPreviewSufaceIsReady(boolean surfaceChanged) {
                LogHelper.i(TAG, "stopRecording onPreviewSufaceIsReady");
    ...
    private void prepareRecording() {
        LogHelper.i(TAG, "[prepareRecording]  ++");
        ...
        LogHelper.i(TAG, "[prepareRecording]  ----mRecordSurface = " + mRecordSurface);
    }
    ...
    private void configreEisValue(Builder requestBuilder) {
        ...
        LogHelper.i(TAG, "configuringRecordingRequests eisValue = " + eisValue);
    ...
    private void restoreCaptureState() {
        LogHelper.d(TAG, "[restoreCaptureState] mIsCapturing = " + mIsCapturing);
        ...
        if (mIsCapturing) {
            mAppController.getActivity().runOnUiThread(new Runnable() {
                ...
                public void run() {
                    mAppUi.setAllCommonViewEnable(true);
                    //If mStopShowCommonUi is true in CameraAppUI, setAllCommonViewEnable(true)
                    //don't set thumbnail view enabled.
                    mAppUi.setThumbnailManagerEnable(true);
                    mAppUi.setSwipeEnabled(true);
                }
            });
    ...
    private void doStopRecording(boolean activityPaused) {
        LogHelper.i(TAG, "[doStopRecording]+ mRecordSurface : " + mRecordSurface);
        mRecordSurface = null;
        if (!activityPaused) {
        ...
        try {
            ...
            e.printStackTrace();
            LogHelper.e(TAG, "doStopRecording with exception:" + e);
            ...
            mAppUi.setAllCommonViewEnable(true);
            LogHelper.i(TAG, "[doStopRecording]-");
        }
    }

    private int getJpegOrientation(byte[] data) {
        LogHelper.d(TAG, "getJpegOrientation, mJpegOrientation = " + mJpegOrientation
                        + ",mOrientation = " + mOrientation + ",mRemoteJpegOrientation = "
                        + mRemoteJpegOrientation);
        int orientation = 0;
        if (ONLY_ONE_CAMERA == mPreviewCameraId.size()) {
            // if camera is not local camera,
            // the orientation is get from mRemoteJpegOrientation
            if (MultiCameraUtil.isLocalSinglePreview(mPreviewCameraId)) {
                orientation = mJpegOrientation;
            } else {
                CameraCharacteristics cs = Utils.getCameraCharacteristics(mActivity,
                                mPreviewCameraId.get(0));
                orientation = Utils.getJpegRotation(mRemoteJpegOrientation, cs);
            }
        } else {
            orientation = mOrientation;
        }
        LogHelper.d(TAG, "getJpegOrientation,return value: " + orientation);
        return orientation;
    }
}

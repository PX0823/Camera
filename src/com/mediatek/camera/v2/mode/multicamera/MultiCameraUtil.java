...
public class MultiCameraUtil {
    private static final Tag TAG = new Tag(
            MultiCameraUtil.class.getSimpleName());
    ...
    protected static ContentValues getCaptureContentValues(int width, int height, int orientation,
         ...
         LogHelper.i(TAG, "[getCaptureContentValues] width = " + width + ",height = " + height
    ...
    protected static ContentValues getVideoContentValues(Activity activity,
        ...
        contentValues.put(Video.Media.TITLE, title);
        contentValues.put(Video.Media.DISPLAY_NAME, filename);
        contentValues.put(Video.Media.DATE_TAKEN, dateTaken);
        contentValues.put(MediaColumns.DATE_MODIFIED, dateTaken / MINUTE_TO_MINI_SECONEDS);
        contentValues.put(Video.Media.MIME_TYPE, mime);
        contentValues.put(Video.Media.DATA, path);
        contentValues.put(Video.Media.WIDTH, profile.videoFrameWidth);
        contentValues.put(Video.Media.HEIGHT, profile.videoFrameHeight);
        contentValues.put(Video.Media.ORIENTATION, orientation);
        contentValues.put(
                Video.Media.RESOLUTION,
                Integer.toString(profile.videoFrameWidth) + "x"
                        + Integer.toString(profile.videoFrameHeight));
        contentValues.put(Video.Media.SIZE, new File(tempPath).length());
    ...
    protected static boolean isLocalSinglePreview(List<String> cameraId) {
        ...
        if (cameraId != null && cameraId.size() == ONLY_ONE_CAMERA) {
            ...
            onlyLocalSinglePreview = Integer.toString(
                    CameraInfo.CAMERA_FACING_BACK).equals(id)
                    || Integer.toString(CameraInfo.CAMERA_FACING_FRONT).equals(
                            id);
        }

        LogHelper.i(TAG, "[isLocalSinglePreview] onlyLocalSinglePreview = "
                + onlyLocalSinglePreview);
    ...
    protected static boolean isLocalCamera(String cameraId) {
        ...
        LogHelper.d(TAG, "[isLocalCamera] : " + localCamera);
        return localCamera;
    }

    protected static Size getRemoteSupportedMaxPictureSize(Activity activity, String cameraId) {
        List<Size> supportedValues = new ArrayList<Size>();
        CameraCharacteristics cameraCharacteristics = Utils.getCameraCharacteristics(activity,
                        cameraId);
        StreamConfigurationMap streamMap = cameraCharacteristics
                        .get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        Size[] highSizes = streamMap.getHighResolutionOutputSizes(ImageFormat.JPEG);
        if (highSizes != null) {
            for (Size size : highSizes) {
                supportedValues.add(size);
            }
        }
        Size[] lowSizes = streamMap.getOutputSizes(ImageFormat.JPEG);
        if (lowSizes != null) {
            for (Size size : lowSizes) {
                supportedValues.add(size);
            }
        }
        // dump the values
        for (Size size : supportedValues) {
            int width = size.getWidth();
            int height = size.getHeight();
            LogHelper.d(TAG, "getRemoteSupportedMaxPictureSize,camera = " + cameraId + ",w = "
                            + width + ",h = " + height);
        }
        // get the biggest value
        Size value = null;
        Size maxSize = null;
        Size maxWidthSize = null;
        Size maxHeightSize = null;
        int maxWidth = -1;
        int maxHeight = -1;
        for (Size size : supportedValues) {
            ...
            value = maxHeightSize.getWidth() * maxHeightSize.getHeight() > maxWidthSize.getWidth()
                            * maxWidthSize.getHeight() ? maxHeightSize : maxWidthSize;
        }
        LogHelper.d(TAG, "getRemoteSupportedMaxPictureSize,value = " + value);
        return value;
    }

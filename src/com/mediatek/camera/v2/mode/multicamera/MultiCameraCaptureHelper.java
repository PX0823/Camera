...
public class MultiCameraCaptureHelper {
    ...
    private static final Tag TAG = new Tag(
            MultiCameraCaptureHelper.class.getSimpleName());
    ...
    public Surface createRemoteCaptureSurface(Size size, int pictureFormat) {
        LogHelper.i(TAG,
                "[createRemoteCaptureSurface]+ size:" + size.getWidth() + "x"
                        + size.getHeight() + ",format = " + pictureFormat);
        checkCreateSufaceCondition(size, pictureFormat);

        // check size, if same size skip
        int width = size.getWidth();
        int height = size.getHeight();
        if (needCreateImageReader(mRemoteImageReader, width, height,
                pictureFormat)) {
            mRemoteSurfaceWidth = width;
            mRemoteSurfaceHeight = height;

            mRemoteImageReader = ImageReader.newInstance(mRemoteSurfaceWidth,
                    mRemoteSurfaceHeight, pictureFormat, MAX_CAPTURE_IMAGES);
            mRemoteImageReader.setOnImageAvailableListener(mCaptureListener,
                    mCaptureHandler);
            mRemoteSurface = mRemoteImageReader.getSurface();
        }

        LogHelper.i(TAG, "[createRemoteCaptureSurface]- mRemoteSurface:"
                + mRemoteSurface);
    ...
    public Surface createSnapShotSurface(Size size, int pictureFormat) {
        LogHelper.i(TAG, "[createSnapShotSurface]+ size:" + size.getWidth() + "x" + size.getHeight()
                + ",pictureFormat = " + pictureFormat);
        checkCreateSufaceCondition(size, pictureFormat);

        // check size, if same size skip
        int width = size.getHeight();
        int heigth = size.getWidth();
        ...
        LogHelper.i(TAG, "[createSnapShotSurface]- mSnapShotSurface:" + mSnapShotSurface);
    ...
    public void setImageCallback(ImageCallback callback) {
        LogHelper.i(TAG, "[setImageCallback] callback = " + callback);
    ...
    private final ImageReader.OnImageAvailableListener mCaptureListener =
        ...
        public void onImageAvailable(ImageReader reader) {
            LogHelper.i(TAG, "[onImageAvailable],mImageCallback = " + mImageCallback);

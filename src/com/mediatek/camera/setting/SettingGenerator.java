...
public class SettingGenerator {
    ...
    private boolean buildPictureSizeEntries(ListPreference pref, List<String> supportedSizes) {
        ...
        for (int i = 0; i < supportedSizes.size(); i++) {
            ...
            if (size.x * size.y == SettingUtils.VGA_SIZE || size.x * size.y == SettingUtils.VGA_SIZE_FULL) {
                entry = "VGA";
            } else if (size.x * size.y == SettingUtils.QVGA_SIZE || size.x * size.y == SettingUtils.QVGA_SIZE_FULL) {
                entry = "QVGA";
            }
